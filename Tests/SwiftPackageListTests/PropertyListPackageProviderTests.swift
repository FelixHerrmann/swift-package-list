...
final class PropertyListPackageProviderTests: XCTestCase {
    func testOutput() throws {
        let packageProvider = PropertyListPackageProvider(bundle: .module, fileName: "example_2")
    ...
    func testErrors() {
        let packageProvider = PropertyListPackageProvider(bundle: .module, fileName: "test")
        XCTAssertThrowsError(try packageProvider.packages()) { error in
            ...
            XCTAssertEqual(cocoaError?.userInfo["file_type"] as? String, "plist")
        }
    }
}
